...
\item Go to http://elm.eu.org and click on the tab ``ELM DB'' to explore the
	...
	ELMs that were manually curated from the literature (Figure
	TP53-BP2-1). This page contains a brief summary of the database
...
\begin{figure}[h!]
	...
	\label{explore_content_elms}
\end{figure}

\item Click on the sub-menu ``ELM classes'' in ``ELM DB'' to see the page with
	all of the ELM classes (Figure TP53-BP2-2). For each class, the
...
\item Search the table for the term ``DOC\_CYCLIN\_1'' and click on its link to
	...
	class (Fig TP53-BP1-4). This page contains a description of the
...
\item Scroll further down the ``DOC\_CYCLIN\_1'' page (Fig TP53-BP1-5) to view
...
%
% Subsection: Browsing annotated instances
%
\subsubsection{Browsing annotated instances}
\label{subsec:explore_content_browsing}
...
\item Click on the sub-menu ``ELM instances'' in ``ELM DB'' to go to the page
	which lists all of the instances in the database (Figure TP53-BP2-3).
...
\item Click on the sub-menu ``ELM pdb structures'' in ``ELM DB'' to see a list
	of all macromolecular structures in the ELM database (Figure
...
\item Click on the sub-menu ``ELM binding domains'' in ``ELM DB'' to see a
	complete list of all the interaction domains in ELM (Figure
...
\begin{figure}[h!]
	...
	\label{explore_content_switches}
\end{figure}

\item Click on the sub-menu ``ELM switches'' in ``ELM DB'' to see a complete
	list of all the switches in ELM (Figure TP53-BP2-7). This table shows
...
\item Click on the sub-menu ``ELM pathways'' in ``ELM DB'' to see a list of all
	pathways contained in ELM (Fig. TP53-BP2-9). Pathways are from the
	``Kyoto Encyclopedia of Genes and Genomes'' (KEGG \cite{26476454})
	database mapped to ELM instances. Click on a species (for example
	``Homo sapiens'') for a complete list of all Human pathways which have
	a protein annotated in ELM, and links to the pathways on KEGG.
...
\begin{figure}[h!]
	...
	\label{fig:explore_contents_viruses}
\end{figure}
...
\item Click on the sub-menu ``ELM virus instances'' in ``ELM DB'' to see a list
	...
	viruses (Fig TP53-BP2-9). The columns are identical to those listed in
	section XXX step YYY (Figure ZZZZ).
