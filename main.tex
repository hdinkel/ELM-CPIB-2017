...
\item Scroll further down the \motif{DOC\_CYCLIN\_1} page
	...
	database (Figure~\ref{fig:explore_content_doc_cyclin_1_abstract_instances})
...
\item Click on the sub-menu \button{ELM methods} under \button{ELM DB} to see a
	...
	(\textit{in vitro}/\textit{in vivo}). Clicking on the link in the ``instances'' column
...
\item Click on the ``ELM Predictions'' button in the menu to access the search
	query page (Figure~\ref{fig:predicting_cv_0974_search}).
...
\begin{figure}[h!]
	...
	conservation score. See steps \ref{sec:predicting_p53_results_phosphorylation}
	to \ref{sec:predicting_p53_results_structure_filter} of
...
\item The Results are summarized in the first figure on the results page
	(see Figure~\ref{fig:predicting_cv_0974_results_summary})
