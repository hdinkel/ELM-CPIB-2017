...
providing the necessary tools to researchers which  
...
for candidate motifs and which helps to discriminate between the most
plausible ones for experimental validation and likely false
...
\item Scroll further down to the ``Switches'' section for a brief overview of
	...
	switches.ELM website where more detail can be found.
...
\item Click on the sub-menu \button{ELM methods} under \button{ELM DB} to see a
	...
	(in vitro, in vivo, in silico, or a combination of these).
...
\item Click on the sub-menu \button{ELM switches} under \button{ELM DB} to see a complete
	...
	from switches.ELM \citep{23550212}.
...
identify proteinal motifs in proteins and sequences. Given a protein's amino acid
...
TODO: Repeat search using stringent filters (homo sapiens, nucleus,
        0.01) Do we want to do this? - Marc (already done in part above)
...
TODO: Marc: read this section to check for consistency, etc.
...
phosphatase (PMID:8866485) whose closest homolog in \textit{C. violaceum} is
...
\item Make sure that the ``Taxonomic Context'' is left empty: If any
    species is used in this field the results will only include motifs which have an
    instance in that species annotated in ELM. For example, if we were to
    fill in ``C. violaceum'', only motifs known to occur in \textit{C.  violaceum}
    would be searched for (which are zero).
...
\item Hit \button{Submit} to send the query to the ELM prediction pipeline.
    ...
    See steps 3 -- 11 of \ref{predicting_p53} for a description of the
...
\item To download a list of all motif instances detected in Human P53, type the
	...
	annotations, including the \fileformat{FASTA}, \fileformat{GFF},
...
\item To download a list of all instances matchin th search query ``CLV'' in
	the yellow fever mosquito (\textit{Aedes aegypti}), enter the following
...
\begin{figure}[h!]
	...
	advanced command that \cl{curl} alone to make the columns align
...
\item Use \cl{curl} to query ELM for all motifs predicted to occur in Human
	...
	instance. The column ``is phiblastmatch'' is True if ?????. The column
	``is filtered'' shows whether or not this motif was rejected by the
	ELM Prediction structure filter. Column ``phibast'' indicates whether
...
TODO: Add: sitches.ELM and phospho.ELM
