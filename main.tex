...
\item The third row shows globular and disordered regions in the sequence as
	...
	(see Commentary section on ELM Filters at page \pageref{DisorderFilter}).
...
\item Use \cl{curl} to query ELM for all motifs predicted to occur in human
	...
	The column ``is annotated'' is True if this motif has been
	annotated in the ELM database as an (experimentally validated) motif
	instance. The column ``is phiblastmatch'' is True if a match was found 
	by the ELM Instance mapper indicating that an experimentally validated 
	instance in a homologous sequence was found.
%TODO: check that this topic is explained in the commentary
%TODO: link to the commentary
	The column ``is filtered'' shows whether or not this motif was rejected by any
...
%TODO: link to taxonfilter in the commentary
...
The ELM structural filter, developed by \cite{19852836}, assesses accessibility
and secondary structural context derived from experimentally solved protein
structures. It maps putatively functional motif occurrences onto a
representative domain structure and scores these motifs for
...
\motif{CLV\_PCSK\_SKI1\_1} example in fig. \ref{fig:predicting_p53_results_summary}).
