...
Each motif class is a short genenerate sequence of amino acids with a given
function. Since the sequences are genenerate, each motif class is represented
...
``[FY].L.P'' is to be read as: The first amino acid is a phenylalaline (F) or a
Tyrosine (Y) followed by any one amino acid (.), a Leucine (L), one more
abritrary amino acid (.) and finally a Proline (P).
...
\section{Searching the ELM database using the REST API}
...
\item To download a list of all motif instances detected in Human P53, type the
	...
	\sdesc{Many other file formats are available for downloading instances annotations, including the \fileformat{FASTA}, \fileformat{GFF}, \fileformat{PIR}, OR \fileformat{PSI-MI} format (either \fileformat{xml} or \fileformat{MiTab})}
...
%%% Commentary 
\section*{Guidelines for Unterstanding
Results}\label{guidelines-for-understanding-results}
